                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            ...
                            <form method="POST" action="{{ route('product.update', $product->id) }}"
                                ...
                                <input type="hidden" name="old_image" value="{{ $product->thumbanail }}"/>
                                ...
                                <div class="row">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="subsubcategory_id" class="form-control">
                                                @foreach ($subsubcategory as $sub)
                                                    <option value="{{ $sub->id }}"
                                                        {{ $sub->id === $product->subsubcategory_id ? ' selected' : '' }}>
                                                        {!! $sub->name !!}</option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="file" name="thumbanail" value="{{ $product->thumbanail }}"
                                                id="thumbanail" class="form-control thumbanail"
                                                onChange="loadFile(event)" />
                                            @error('thumbanail')
                                                <p class="text-danger">{{ $message }}</p>
                                            @enderror

                                            <div class="bg-img h-40 w-40 mt-20"
                                                style="background-image: url('{{ asset($product->thumbanail) }}')"></div>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="current_status" class="form-control">
                                                ...
                                                <option value="active" {{ $product->current_status == 'active' ? 'selected' : '' }}>Active</option>
                                                <option value="Inactive" {{ $product->current_status == 'inactive' ? 'selected' : '' }}>Inactive</option>
                                            </select>
                            ...
                            @if(count($multimages) > 0)
                                <div class="col-md-12 mt-4">
                                    <div class="box bt-3 border-info">
                                        <h4 class="box-header box-title">Product Multiple Images Update</h4>
                                        <div class="box-body">
                                            <form method="post" action="{{ route('product.update.image') }}" enctype="multipart/form-data">
                                                @csrf
                                                <div class="row row-sm">

                                                    @foreach ($multimages as $img)
                                                        <div class="col-md-3 card">
                                                            <div class="thumbnail mt-2" style="background-image: url('{{ asset($img->photo_name)}}'); height: 120px; width: 100%; margin-right: 20px;background-size: cover;background-position: center center;"></div>
                                                            <div class="card-body">
                                                                <form method="post" action="{{ route('product.multimage.delete', $img) }}">
                                                                    @csrf
                                                                    @method('PUT')
                                                                    <button type="submit" class="btn btn-sm btn-danger" href=""><i class="fa fa-trash"></i></button>
                                                                </form>
                                                                <p class="mt-3">Change Image '{{ $img->id }}'</p>
                                                                <div class="form-group">
                        ...
                        success: function(data) {
                            ...
                            $.each(data, function(key, value) {
                                $('select[name="subsubcategory_id"]').append(
                                    '<option class="{{ $product->subcategory_id }}" value="' +
